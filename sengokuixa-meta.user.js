...
if ( Env.chapter <= 5 ) {
	...
	data = {
		...
		'大筒兵':   { type: 335, class: 'heiki3', attack: 10, defend: 12, speed:  8, destroy:  8, command: '器', skillType: '器', training: [ 270, 217, 174, 140, 113, 91, 73, 59, 48, 39, 32, 26, 22, 18, 15 ], dou: 300, require: ['弓', '器'], order: 3 },
...
var Unit = function( $form, type ) {
	...
	if ( $form && $form.find('#howto_butai_hensei').length == 0 ) {
		var villagename = $form.find('.ig_deck_unitdata_assign').text().trim(),
			condition = $form.find('.ig_deck_unitdata_condition').text().trim(),
